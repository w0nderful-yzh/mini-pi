...
| M7.7 修补 | 验收后发现的可用性问题：`/` 补全菜单只在唯一匹配时由 Enter 先补全（第二次 Enter 才提交），避免半截命令变成 `unknown command`；交互终端缺 `prompt_toolkit` 时在 REPL 顶部打印降级原因，不再静默失去补全/历史。同时修正用户 `uv tool` 安装缺依赖的环境（`uv tool upgrade mini-pi`） | 新增 4 passed（唯一匹配 Enter 先补全、多匹配仍提交原文、完整命令一次提交、降级提示可见）；全量 542 passed, 5 deselected（`NO_COLOR` 清除、`TERM=xterm-256color`）；`2959242` |
