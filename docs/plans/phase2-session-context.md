...
> 状态：实施中。M7.1–M7.4、M7.C1–C8、M7.5a–M7.5b 已完成；下一任务是 **M7.5c：准备 Compaction 输入**。本文件先列待开发任务，已完成交付放在末尾。
...
| M7.5b | 固定摘要协议与单次调用器：Goal、Constraints、Progress、Key Decisions、Next Steps、Critical Context、read/modified files 模板；独立 system prompt、`tools=None`；空摘要、`length`、error 状态、意外 tool call 与 LLM error 一律失败不落盘 | `tests/context/test_summarizer.py` 9 passed；全量 442 passed, 3 deselected（`NO_COLOR` 清除、`TERM=xterm-256color`）；本提交 |
...
4. 下一批只做 **M7.5c**；序列化与摘要调用已固化，先用活动投影和切点生成不可变 plan，仍不写盘。
